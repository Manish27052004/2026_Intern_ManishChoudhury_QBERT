...
# -*- coding: utf-8 -*-
"""
Q-BERT Quantization | DistilBERT × RTE
File   : 2026_intern_ManishChoudhury_QBERT_distilbert_rte.py
...
def evaluate(model, tokenizer, dataset, batch_size=64):
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(dataset), batch_size), desc="  Evaluating", leave=False):
            ...
            t1s = [str(t) for t in b["sentence1"]]; t2s = [str(t) for t in b["sentence2"]]
    ...
    pf, lf = zip(*valid)
    acc = 100.0 * sum(p == l for p, l in zip(pf, lf)) / len(lf)
    return acc, list(pf), list(lf)
...
def compute_metrics(preds, labels, lat_ms):
    ...
    throughput = round(1000.0 / lat_ms, 4)
    energy     = round(GPU_ACTIVE_POWER_W * (lat_ms / 1000.0) * 1000.0, 4)
    return p, r, f1, throughput, energy
...
def main():
    ...
    val_ds = load_dataset("nyu-mll/glue", "rte", split="validation")
    # RTE is small, use all
    ...
    fp32_lat  = measure_latency(model_fp32, tokenizer, val_ds["sentence1"], val_ds["sentence2"])
    ...
    q_lat  = measure_latency(model_q, tokenizer, val_ds["sentence1"], val_ds["sentence2"])
    ...
    pd.DataFrame([{
        "model":"distilbert-base-uncased","task":"rte","checkpoint":CHECKPOINT,
