...
# -*- coding: utf-8 -*-
"""
Q-BERT Quantization | DistilBERT × SST-2
File   : 2026_intern_ManishChoudhury_QBERT_distilbert_sst2.py
...
WEIGHT_BIT  = 8
MAX_LEN     = 128
RESULTS_CSV = "result_distilbert_sst2.csv"
LATENCY_WARMUP, LATENCY_RUNS, LATENCY_BATCH = 20, 100, 32
GPU_ACTIVE_POWER_W = 210  # ~60% of L40S TDP (350W)

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("="*60)
print("  Q-BERT | DistilBERT × SST-2")
print("="*60)
...
def measure_latency(model, tokenizer, texts):
    model.eval()
    enc = tokenizer([str(t) for t in texts[:LATENCY_BATCH]], padding=True,
...
def evaluate(model, tokenizer, dataset, batch_size=64):
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(dataset), batch_size), desc="  Evaluating", leave=False):
            ...
            enc = tokenizer([str(t) for t in b["sentence"]], padding=True,
                            truncation=True, max_length=MAX_LEN, return_tensors="pt")
            enc = {k: v.to(DEVICE) for k, v in enc.items()}
...
def main():
    ...
    fp32_lat  = measure_latency(model_fp32, tokenizer, val_ds["sentence"])
    ...
    q_lat  = measure_latency(model_q, tokenizer, val_ds["sentence"])
    ...
    pd.DataFrame([{
        "model":"distilbert-base-uncased","task":"sst2","checkpoint":CHECKPOINT,
