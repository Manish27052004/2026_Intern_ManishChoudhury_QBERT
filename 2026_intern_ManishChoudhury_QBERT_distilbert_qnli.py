...
# -*- coding: utf-8 -*-
"""
Q-BERT Quantization | DistilBERT × QNLI
File   : 2026_intern_ManishChoudhury_QBERT_distilbert_qnli.py
...
def evaluate(model, tokenizer, dataset, batch_size=64):
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(dataset), batch_size), desc="  Evaluating", leave=False):
            ...
            t1s = [str(t) for t in b["question"]]; t2s = [str(t) for t in b["sentence"]]
...
def main():
    ...
    val_ds = load_dataset("nyu-mll/glue", "qnli", split="validation")
    ...
    fp32_lat  = measure_latency(model_fp32, tokenizer, val_ds["question"], val_ds["sentence"])
    ...
    q_lat  = measure_latency(model_q, tokenizer, val_ds["question"], val_ds["sentence"])
    ...
    pd.DataFrame([{
        "model":"distilbert-base-uncased","task":"qnli","checkpoint":CHECKPOINT,
