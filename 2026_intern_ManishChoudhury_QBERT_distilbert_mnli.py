...
# -*- coding: utf-8 -*-
"""
Q-BERT Quantization | DistilBERT × MNLI
File   : 2026_intern_ManishChoudhury_QBERT_distilbert_mnli.py
...
def evaluate(model, tokenizer, dataset, batch_size=64):
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(dataset), batch_size), desc="  Evaluating", leave=False):
            ...
            t1s = [str(t) for t in b["premise"]]; t2s = [str(t) for t in b["hypothesis"]]
...
def main():
    ...
    val_ds = load_dataset("nyu-mll/glue", "mnli", split="validation_matched")
    ...
    fp32_lat  = measure_latency(model_fp32, tokenizer, val_ds["premise"], val_ds["hypothesis"])
    ...
    q_lat  = measure_latency(model_q, tokenizer, val_ds["premise"], val_ds["hypothesis"])
    ...
    pd.DataFrame([{
        "model":"distilbert-base-uncased","task":"mnli","checkpoint":CHECKPOINT,
